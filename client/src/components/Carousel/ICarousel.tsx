...
export default function ICarousel() {
  ...
  const slide_text = [
    <>
      POWER 스위치를 ON으로 한 후 HEAT 스위치를 ON으로 하면 10초 공회전 후 점화
      됩니다.
      <br />
      이때 온도가 상승하며 현재의 온도가 표시 됩니다.
      <br />
      점화가 안되고 Err표시가 되면서 알림이 울리면 가스공급 유무를 확인하여
      주십시오.
    </>,
    <>
      설정온도 표시는 HEAT 스위치의 ON,OFF 에 상관없이 점화중에도 UP 또는 DOWN
      버튼으로 온도의 설정, 변경이 가능합니다. <br /> 이때 UP 이나 DOWN 버튼을
      누르면 3초간 깜빡이며 설정되어진 온도가 표시되고 곧 현재온도로 전환된다.{" "}
      <br />
      버튼을 떼면 오븐기 내부의 현재온도가 표시 됩니다.
    </>,
    <>
